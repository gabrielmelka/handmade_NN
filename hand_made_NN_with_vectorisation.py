...
#%% generation of parameters and resizing data
...
training_length=2000 #number of iterations in the training
...
for k in range(training_length):
    # the scoring
    if (k%50==0 and k<500) or k <10 or (k<100 and k%5==0) or k%200==0 or k==training_length :
        accuracy=0
    ...
    print(f"Iteration {k+1}/{training_length} terminée en {elapsed_iter:.2f}s")
...
plt.plot(List_L, 'o', color='tab:blue', markersize=4, label='List_L')
plt.axhline(0, color='gray', linestyle='--', linewidth=1)
plt.title("Graphique de List_L")
...
# Deuxième graphique
plt.figure(figsize=(8,5))
plt.plot(np.array(time_scoring), np.array(accuracy_scores)/len_test*100, 'o', color='tab:green', markersize=3, label='Accuracy (%)')
plt.axhline(10, color='red', linestyle='--', linewidth=1, label='10% limite')
plt.axhline(100, color='orange', linestyle='--', linewidth=1, label='100%')
plt.title("Scores d'accuracy sur toutes les itérations")
plt.xlabel("Itérations")
plt.ylabel("Accuracy (%)")
plt.grid(True, linestyle=':', linewidth=0.5)
plt.legend()
